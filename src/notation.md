...
次に、アルファベットや記号の並びをクォートで囲んだものも、同様に扱います。これは後述するように、
...
その要素はオプショナルであるということになります。

以下は、 `else` で始まり、その次に `<式>` が来るような要素が0回または1回現れることを意味します。
...
曖昧さを解決するために、明示的に `'|'` としない限り、 `a` または `b` という解釈が優先されます。
...
なお、Scala 2から続く `if '(' <条件式> ')' <式> ( else <式> )?` という形式も引き続き有効です。
