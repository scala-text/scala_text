...
ここで、`PostalCodeResult` 全体は `sealed trait` として定義し、成功ケース `Success` は通常の `case class`、失敗ケースは `enum Failure` でまとめています。`enum Failure extends PostalCodeResult` と書くことで、`Failure` の各caseも `PostalCodeResult` のサブ型として扱えます。失敗ケースだけを `enum` で平坦に並べることで網羅性チェックの恩恵を受けながら、成功と失敗を `PostalCodeResult` の階層で区別できるという、Scala 3ならではの簡潔な書き方です。
