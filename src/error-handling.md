...
しばしば返り値をnullかどうかチェックするのを忘れて実行時エラーの[NullPointerException](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/lang/NullPointerException.html)（通称：ぬるぽ・NPE）を発生させてしまいます。
...
### [Option](https://github.com/scala/scala3/blob/3.8.4/library/src/scala/Option.scala)
...
その中身の値で`str`という別の変数を束縛することもできます。

中身を取りだすのではなく、中身で変数を束縛するというテクニックは、`List`のパターンマッチでも
...
for式は実際には`flatMap`と`map`に展開されて実行されるのです。
...
### [Either](https://github.com/scala/scala3/blob/3.8.4/library/src/scala/util/Either.scala)
...
そしてLeftに用いるエラー値ですが、これは **代数的データ型（algebraic data type）** で定義するとよいでしょう。Scala 3では `enum` で簡潔に書けます[^adt-scala2]。
パターンマッチの節で解説したように代数的データ型を用いることでエラーの処理が漏れているかどうかをコンパイラが検知してくれるようになります。
単に`Throwable`型をエラー型に使うのなら後述のTryで十分です。

例としてEitherを使ってログインのエラーを表現してみましょう。Leftの値となる`LoginError`を `enum` で定義します。

```scala
enum LoginError {
  case InvalidPassword  // パスワードが間違っている場合のエラー
  case UserNotFound     // nameで指定されたユーザーが見つからない場合のエラー
  case PasswordLocked   // パスワードがロックされている場合のエラー
}
```
...
-- [E029] Pattern Match Exhaustivity Warning: -----------------------------
  ...
  |
  |It would fail on pattern case: Left(UserNotFound), Left(PasswordLocked)
```

のようにコンパイラが`Left(UserNotFound)`と`Left(PasswordLocked)`の処理が漏れていることをwarningで教えてくれます。
...
### [Try](https://github.com/scala/scala3/blob/3.8.4/library/src/scala/util/Try.scala)
...
#### [`NonFatal`](https://github.com/scala/scala3/blob/3.8.4/library/src/scala/util/control/NonFatal.scala)の例外

`Try.apply`がcatchするのはすべての例外ではありません。
NonFatalという種類の例外だけです。
NonFatalではない例外はアプリケーション中で復旧が困難な非常に重度なものです。
なので、NonFatalではない例外はcatchせずにアプリケーションを終了させて、外部から再起動などをしたほうがいいです。
...
object MainBefore {
  ...
  enum Failure extends PostalCodeResult {
    case UserNotFound, UserNotHasAddress, AddressNotFound, AddressNotHasPostalCode
  }
  import Failure.*
...
ここで、`PostalCodeResult` 全体は `sealed trait` として定義し、成功ケース `Success` は通常の `case class`、失敗ケースは `enum Failure` でまとめています。`enum Failure extends PostalCodeResult` と書くことで、`Failure` の各caseの値も `PostalCodeResult` 型の値として扱えます。失敗ケースだけを `enum` で平坦に並べることで網羅性チェックの恩恵を受けながら、成功と失敗を `PostalCodeResult` の階層で区別できるという、Scala 3ならではの簡潔な書き方です。
...
[^adt-scala2]: Scala 2では `enum` がなく、同じ意図のデータ型を `sealed trait`（または `sealed abstract class`）と複数の `case class`/`case object` の組み合わせで表現していました。`enum` で書けないネストした階層を作りたい場合は、Scala 3でも `sealed trait` の方が向いています。
