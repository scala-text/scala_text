...
[`Function0`](https://github.com/scala/scala3/blob/3.8.4/library/src/scala/Function0.scala) 〜 [`Function22`](https://github.com/scala/scala3/blob/3.8.4/library/src/scala/Function22.scala) までのトレイトの無名サブクラスのインスタンスなのです。
...
また、Scalaではメソッドを「関数値」に変換することができ、メソッドの引数リストを複数に分けてあれば、その変換によってカリー化された関数値が得られます。Scala 3ではメソッド名をそのまま書くだけで自動的に関数値に **η展開（eta-expansion）** されます。REPLで確認してみましょう。
