...
これを `Int => Int => Int` 型の関数のように、1つの引数を取り、残りの引数を取る関数を返す関数のチェーンで表現するというものです。
...
また、Scalaではメソッドの引数リストを複数に分けることで簡単にカリー化された関数を得ることができます。
...
引数リストを複数に分けた`addCurried`の型は1引数関数のチェーンになっていて、確かにカリー化されています。
