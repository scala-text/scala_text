...
他にも、たとえばScalaの「case class」や「enum」は、オブジェクト指向の視点ではValue Objectパターンに近いものと考えられますが、関数型の視点では代数的データ型として考えることができます。
また「given/using」（Scala 2の「implicit parameter」に相当）はオブジェクト指向の視点では暗黙的に型で解決される引数に見えますが、関数型の視点ではHaskellの型クラスに近いものと見ることができます。
...
- traitによるmixin（Scala 3では`trait`にパラメータを持たせる *trait parameters* もサポート）
- 構造的部分型
- 型パラメータの変位（variance）指定
- self type annotation（`self: A & B`）による静的な依存性の注入
