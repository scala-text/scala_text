...
[リファクタリング前のプログラム](https://github.com/dwango/scala_text/tree/master/example_projects/trait-example)

[リファクタリング後のプログラム](https://github.com/dwango/scala_text/tree/master/example_projects/trait-refactored-example)
...
既にメソッドの実装を説明したことがある場合は実装を省略し[`???`](https://github.com/scala/scala/blob/v2.11.7/src/library/scala/Predef.scala#L222-L225)で書くことがあります。
...
note top of UserRepository : インタフェース
note bottom of UserRepositoryImpl : サービス
note top of UserService : クライアント
note bottom of UserServiceImpl : インジェクタ（？）

hide members

UserRepositoryImpl -u-|> UserRepository
UserService -r-|> UserRepository
UserServiceImpl -r-|> UserRepositoryImpl
