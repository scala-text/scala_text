...
```
ワイルドカードインポートはScala 3では`*`、Scala 2では`_`を使います。
...
と記述することができます。クラスの章で説明したとおり、Scala 3では`new`を省略してインスタンスを生成できます（`new ArrayList[String]()`のように`new`を付けて書くこともできます）。
...
現在時刻をミリ秒単位で取得する[`System.currentTimeMillis()`](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/lang/System.html#currentTimeMillis%28%29)をScalaから呼び出してみましょう。

```
scala> System.currentTimeMillis()
val res0: Long = 1416357548906
```
...
継承されない、ということです。たとえば、Javaでは [`JFrame.EXIT_ON_CLOSE`](https://docs.oracle.com/en/java/javase/17/docs/api/java.desktop/javax/swing/JFrame.html) が継承されることを利用して、
...
[`scala.collection.mutable.ArrayBuffer`](https://www.scala-lang.org/api/current/scala/collection/mutable/ArrayBuffer.html)型の値を生成してから、`scala.jdk.CollectionConverters`を使って[java.util.List](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/List.html)型に変換してみましょう。なお、`ArrayBuffer`には1つ以上の要素を入れておくこととします。
...
この機能に対応するものとして、Scalaにはワイルドカード型があります。Scala 2ではこの機能は存在型と呼ばれる機能で表現されていましたが、Scala 3では存在型は削除され、`?`を使ったワイルドカード型を使います。上記のJavaコードは、Scalaでは次のコードで表現することができます。

```scala mdoc:nest
import java.util.{List as JList, ArrayList as JArrayList}

val objects: JList[? <: Object] = new JArrayList[String]()
```

```scala mdoc:nest
import java.util.{Comparator as JComparator}
...
Scala 2.12ではSAM(Single Abstract Method)変換が導入され[^sam]、Java 8のラムダ式を想定したライブラリを簡単に利用できるようになりました。この機能はScala 3にもそのまま引き継がれています。
...
[^sam]: 正確には`-Xexperimental`オプションにより、Scala 2.11でもSAM変換を有効にすることができます。
