...
`if`式はJavaの`if`文とほとんど同じ使い方をします。Scala 3では`then`キーワードを使って条件式の括弧を省略する書き方ができるようになっており、本テキストではこの書き方を使います。

```scala
if <条件式> then <then式> (else <else式>)?
```

Scala 2から続く括弧つきの構文も引き続き利用できます。
...
なお、`else`が省略された場合、`if`式全体は`Unit`型になります。条件が`true`のときは`then式`が評価されますがその値は捨てられ、条件の真偽にかかわらず式の値は`Unit`型の唯一の値`()`になります。`Unit`型はJavaでは`void`に相当するもので、返すべき値がないときに使われます。
...
if 1 <= age && age <= 6 && !isSchoolStarted then {
  println("幼児です")
} else {
  println("幼児ではありません")
}
...
Scala 3の`while`式は`do`キーワードで本体を導入します。
...
Javaで`while`文を使った場合と同様です。なお、Scala 2にあった`do { ... } while (...)`形式の構文はScala 3で廃止されているため、本テキストでは扱いません。Javaの`break`文や`continue`文に相当する言語機能はありませんが、後ほど説明する高階関数を適切に利用すれば、ほとんどの場合、`break`や`continue`は必要ありません。
...
Scala 3の`for`式は`do`または`yield`で本体を導入する形が基本です。

```
for (<ジェネレータ>;)+ do <本体式>
for (<ジェネレータ>;)+ yield <本体式>
# <ジェネレータ> = x <- <式> (if <条件式>)?
```
...
```scala mdoc:nest
for x <- 1 to 5; y <- 1 until 5 if x != y do {
  println("x = " + x + " y = " + y)
}
```
...
```scala mdoc:nest
for e <- List("A", "B", "C", "D", "E") do println(e)
```
...
```scala
for a <- 1 to 1000; b <- 1 to 1000; c <- 1 to 1000 if a * a == b * b + c * c do {
  println((a, b, c))
}
```
...
たとえば、以下の様なパターンマッチをREPLで実行しようとすると、2つの警告が出てしまいます。
...
1つ目の警告の意味はScalaコンパイラの「型消去」という動作により`List[Int]`の`Int`の部分が消されてしまうのでチェックされないということです。

結果的に2つのパターンは区別できないものになり、パターンマッチは上から順番に実行されていくので、2番目のパターンは到達しないコードになります。2つ目の警告はこれを意味しています。
...
## optional braces（インデントで書く制御構文）

ここまで紹介した`if 条件 then ... else ...` / `while 条件 do ...` / `for ... do/yield ...`はScala 3の **新しい制御構文** で、本テキストでは主役として採用しています。Scala 2から続く括弧つき構文（`if (条件) { ... }`など）も引き続き有効ですので、既存コードを読む際に混在していても困らないでしょう。

Scala 3ではさらに、`{}`を省略して **インデント** でブロックを表現する「optional braces（省略可能な波括弧）」もあります。`then`/`do`/`yield`/`=`の後に続く本体をインデントで揃えれば、波括弧を書かずに済みます。
...
本テキストではブロックの境界を明示するために波括弧を残していますが、Scala 3のプロジェクトではoptional bracesで書かれたコードもよく目にするはずです。どちらのスタイルでも意味は同じなので、プロジェクトのスタイルに合わせて選んでください。
