...
この定義は古典と呼ばれている書籍『ソフトウェアテストの技法　第二版』をベースにしたものです。
...
そしてリファクタリングを行う際に、ソフトウェアの使用を変えなかったことを確認する手段としてユニットテストが必要になるのです。
...
下記の仕様のを満たすCalcクラスを作成し、それらをテストしていきます。
...
そんな作業を、限られたリソースですべてのパッチに対して行うのは不可能に近いので、OSSによってはテストコードのないパッチはレビュー対象から外すことが多いです。
