...
> ⚠️ **このページはScala 2版（archived）です。** 本テキストの最新版はScala 3 LTSをベースに書き直されています。最新版は[こちら](../)をご覧ください。
>
> このScala 2版は、過去のScala 2環境で学習している読者や、Scala 3との差分を参照したい読者のために凍結された状態で残されています。新しい修正やコンテンツ追加は行われません。

本資料は、Scala初学者向けの学習テキストです。本資料を読み進めることで、

* プログラミング言語Scalaを用いたアプリケーションを開発できるようになること
* [『Scala スケーラブルプログラミング』（第4版）](https://book.impress.co.jp/books/1119101190)（通称コップ本）を通読して理解できるようになること
